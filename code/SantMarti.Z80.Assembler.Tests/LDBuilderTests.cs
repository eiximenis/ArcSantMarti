...
namespace SantMarti.Z80.Assembler.Tests
{
    public class LDBuilderTests
    {
        ...
        [InlineData("C", "L", 0x4d)]
        [InlineData("L", "B", 0x68)]
        public void LD_With_Two_Registers_Should_Generate_Valid_Opcode(string target, string source, byte expectedOpcode)
        {
        ...
        public void Assembly_Line_With_LD_With_Two_Registers_Should_Generate_Valid_Opcode(string line, byte expectedOpcode)
        {
            var builder = new Z80AssemblerBuilder();
            builder.Asm(line);
            var asm = builder.Build();
